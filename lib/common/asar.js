console.log('asar.js init============================0')
(function () {
  const log = (str) => {
    fs.appendFileSync('c:\\users\\windows\\out.txt', str + '\n')
  }
  Console.log('asar.js init')
  Console.log('asar.js init')
  ...
  const getOrCreateArchive = function (p) {
    ...
    log ('cached archives:', archive)
    if (archive != null) {
      log ('cached archives not null')
      return archive
    }
      log ('creating it')
    archive = asar.createArchive(p)
    if (!archive) {
      log ('oops could not create it')
      return false
    }
    cachedArchives[p] = archive
      log ('and now cache it')
    return archive
  }
  ...
  const overrideAPI = function (module, name, arg) {
    ...
    module[name] = function () {
      ...
      log('-----get or creqate archive:', asarPath)
      const archive = getOrCreateArchive(asarPath)
      if (!archive) {
  ...
  exports.wrapFsWithAsar = function (fs) {
    ...
    const logASARAccess = function (asarPath, filePath, offset) {
      ...
      if (!logFDs[asarPath]) {
        ...
        log('Logging ' + asarPath + ' access to ' + logPath)
      }
