...
ipcMain.on('chrome-windows-get-current', function (evt, responseId, getInfo) {
  ...
  if (getInfo) {
    if (getInfo.populate) {
      // TODO(bridiver) - implement for last pass
...
ipcMain.on('chrome-windows-get-all', function (evt, responseId, getInfo) {
  ...
  var response = electron.BrowserWindow.getAllWindows().map((win) => {
    var win_info = {
      focused: false,
      incognito: false // TODO(bridiver)
    }
    win_info.id = win.id
    win_info.focused = win.isFocused()
    var bounds = win.getBounds()
    win_info.top = bounds.y
    win_info.left = bounds.x
    win_info.width = bounds.width
    win_info.height = bounds.height
    win_info.alwaysOnTop = win.isAlwaysOnTop()
    return win_info
  })
