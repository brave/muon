...
app.startWorker = function (module_name) {
  const id = app._startWorker(module_name)
  ...
  app.on('worker-post-message', (e, worker_id, message) => {
    if (worker.id === worker_id) {
      ...
      worker.onmessage && worker.onmessage(message)
    }
  })
  app.on('worker-onerror', (e, worker_id, message) => {
    worker.lastError = message
    if (worker.id === worker_id) {
      worker.onerror && worker.onerror(message)
    }
  })
