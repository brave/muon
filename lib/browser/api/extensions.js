...
const createTab = function (createProperties, sender, responseId) {
  ...
  console.log(sender.getURL())
  const senderHostWebContents = sender.isGuest() ? sender.senderHostWebContents : sender
  const senderWin = BrowserWindow.fromWebContents(senderHostWebContents)

  if (windowId === -2) {
    win = BrowserWindow.getActiveWindow() || senderWin
    if (!win) {
  ...
  if (error) {
    ...
    if (!sender.isDestroyed()) {
      sender.send('chrome-tabs-create-response-' + responseId, null, error)
    }
  }
  ...
  try {
    ...
    webContents.createTab(
      ...
      (tab) => {
        if (!sender.isDestroyed()) {
          if (tab) {
            sender.send('chrome-tabs-create-response-' + responseId, extensions.tabValue(tab))
          } else {
            sender.send('chrome-tabs-create-response-' + responseId, null, "An unexpected error occurred")
          }
        }
      }
    )
    ...
    console.error(e)
    sender.send('chrome-tabs-create-response-' + responseId, null, "An unexpected error has occured: " + e.message)
  }
}
...
const chromeTabsUpdated = function (tabId) {
  let oldTabInfo = getTabValue(tabId)
  if (!oldTabInfo) {
    return
  }
...
app.on('web-contents-created', function (event, tab) {
  if (extensions.isBackgroundPage(tab)) {
  ...
  tab.on('set-active', function (evt, active) {
    ...
    chromeTabsUpdated(tabId)
    var win = tab.getOwnerBrowserWindow()
    if (win && active) {
