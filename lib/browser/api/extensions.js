...
const ReadableIpcStream = require('../../common/readable-ipc-stream')
...
app.on('web-contents-created', function (event, tab) {
  ...
  tab.on('tab-detached-at', function () {
    chromeTabsUpdated(tabId)  // immediate
  })
...
ipcMain.on('chrome-tabs-create', function (evt, responseId, createProperties) {
  const cb = (tab, error) => {
    if (!evt.sender.isDestroyed()) {
      evt.sender.send('chrome-tabs-create-response-' + responseId, tab.isDestroyed() ? null : tab.tabValue(), error)
    }
  }
  try {
...
ipcMain.on('chrome-tabs-remove', function (evt, responseId, tabIds) {
  const senderTabId = evt.sender.isDestroyed() ? -1 : evt.sender.getId()
  ...
  if (senderTabId !== -1 && tabIds.indexOf(senderTabId) === -1) {
...
ipcMain.on('register-protocol-stream-handler', function (evt, scheme) {
  ...
  protocol.registerStreamProtocol(scheme, (request, cb) => {
    ...
    const rs = new ReadableIpcStream(ipcMain, `chrome-protocol-stream-handled-${requestId}-stream`)

    ipcMain.once(`chrome-protocol-stream-handled-${requestId}-headers`, (e, data) => {
      cb({
