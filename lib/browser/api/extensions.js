...
const getOpener = function (createProperties) {
  let opener = null
  if (createProperties.openerTabId) {
    opener = getWebContentsForTab(createProperties.openerTabId)
    assert(opener, 'No tab with id: ' + createProperties.openerTabId)
  }

  // use the active tab if an openerTabId wasn't specified
  if (!opener) {
    let activeTab = getActiveTab()
    opener = activeTab && getWebContentsForTab(activeTab.id)
  }

  return opener
}

const registerGuest = function (createProperties) {
  const guestInstanceId = guestViewManager.getNextInstanceId()

  let opener = getOpener(createProperties)
  let embedder = null
  if (createProperties.windowId) {
    let win = BrowserWindow.fromId(createProperties.windowId)
    embedder = win.webContents
    assert(embedder, 'No window with id: ' + createProperties.windowId)
    if (createProperties.tabId) {
      assert(opener.hostWebContents === embedder, 'No tab with id: ' + createProperties.tabId)
    }
  } else {
    embedder = opener && (opener.hostWebContents || opener)
    // TODO(bridiver) last active window if none are focused
    let win = BrowserWindow.getFocusedWindow() || BrowserWindow.getAllWindows()[0]
    embedder = embedder || (win && win.webContents)
    assert(embedder, 'No window availale for tab')
  }

  let guestSession = opener ? opener.session : session.defaultSession
  var options = {
    guestInstanceId,
    embedder,
    session: guestSession,
    isGuest: true,
    delayedLoadUrl: createProperties.url
  }
  const guest = webContents.create(options)
  guestViewManager.registerGuest(guest, guestInstanceId)
  return guest
}

var pending_tabs = {}

const openTab = function (guest, createProperties, sender) {
...
const createTab = function (createProperties, sender, responseId) {
  const guest = registerGuest(createProperties)
  ...
  let opener = getOpener(createProperties) || sender
  openTab(guest, createProperties, opener)
}
...
const initializeTab = function (tabId, webContents) {
...
ipcMain.on('chrome-tabs-create', function (evt, responseId, createProperties) {
  try {
    ...
    evt.sender.send('chrome-tabs-create-response-' + responseId, null, e.message)
  }
})
...
exports.tabValue = extensions.tabValue
exports.isBackgroundPage = extensions.isBackgroundPage
