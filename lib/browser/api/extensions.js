...
ipcMain.on('register-protocol-stream-handler', function (evt, scheme) {
  console.log('register-protocol-stream-handler', scheme, evt)
  const sender = evt.sender
  if (evt.sender.isDestroyed()) {
    return
  }
  ...
  protocol.registerStreamProtocol(scheme, (request, cb) => {
    ...
    if (sender.isDestroyed()) {
      ...
      const ipcStream = new IpcStream(`chrome-protocol-stream-handled-${scheme}-${requestId}-stream`, BrowserWindow)
      ipcMain.once(`chrome-protocol-stream-handled-${scheme}-${requestId}`, (evt, data) => {
        console.log(`chrome-protocol-stream-handled-${scheme}-${requestId}`, {evt, data})
        cb({
