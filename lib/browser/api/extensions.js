...
var sendToBackgroundPages = function (extensionId, event, session) {
...
var chromeTabsUpdated = function (tabId, changeInfo) {
  ...
  if (tabValue) {
    sendToBackgroundPages('all', 'chrome-tabs-updated', getSessionForTab(tabId), changeInfo, tabValue)
  }
}
...
var chromeTabsRemoved = function (tabId) {
  ...
  delete tabs[tabId]
  sendToBackgroundPages('all', 'chrome-tabs-removed', session, {
    windowId: windowId,
...
var initializeTab = function (tabId, webContents) {
  ...
  if (pending) {
    ...
    try {
      pending.sender.send('chrome-tabs-create-response-' + pending.responseId, pending.tabInfo)
    } catch (e) {}
    delete pending_tabs[tabId]
  }
  sendToBackgroundPages('all', 'chrome-tabs-created', getSessionForTab(tabId), extensions.tabValue(webContents))
}
...
app.on('web-contents-created', function (event, webContents) {
  ...
  webContents.on('did-finish-load', function () {
    initializeTab(tabId, webContents)
    chromeTabsUpdated(tabId, {status: 'complete', url: webContents.getURL()})
  })
  webContents.on('set-active', function (evt, active) {
    initializeTab(tabId, webContents)
    var win = webContents.getOwnerBrowserWindow()
    if (win && active)
      sendToBackgroundPages('all', 'chrome-tabs-activated', getSessionForTab(tabId), {windowId: win.id})
  })
  webContents.on('destroyed', function () {
    chromeTabsRemoved(tabId)
  })
...
ipcMain.on('chrome-browser-action-clicked', function (evt, extensionId, tabId, name, props) {
  ...
  if (popup) {
    ...
    if (response) {
      sendToBackgroundPages(extensionId, 'chrome-browser-action-clicked', getSessionForTab(tabId), response)
    }
  }
})
