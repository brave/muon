...
const createTab = function (createProperties, cb) {
  ...
  if (!error && createProperties.partition) {
    ...
    // don't pass the partition through
...
const chromeTabsUpdated = function (tabId) {
  let oldTabInfo = tabs[tabId].tabValue
  if (!oldTabInfo) {
    return
  }
...
ipcMain.on('chrome-tabs-create', function (evt, responseId, createProperties) {
  const cb = (tab, error) => {
    if (!evt.sender.isDestroyed()) {
      evt.sender.send('chrome-tabs-create-response-' + responseId, extensions.tabValue(tab), error)
    }
  }
  try {
