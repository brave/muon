...
const {extension, Extension} = process.atomBinding('extension')

Object.setPrototypeOf(Extension.prototype, EventEmitter.prototype)
...
var getTabValue = function (tabId) {
  ...
  if (webContents) {
    return extension.tabValue(webContents)
  }
}
...
var createTab = function (createProperties, sender, responseId) {
  ...
  if (opener.isGuest()) {
    ...
    let tabInfo = extension.tabValue(guest)
...
var initializeTab = function (tabId, webContents) {
  ...
  tabs[tabId].tabInfo = extension.tabValue(webContents)
...
app.on('web-contents-created', function (event, webContents) {
  if (extension.isBackgroundPage(webContents)) {
