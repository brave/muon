...
const IpcStream = require('../../common/electron-ipc-stream')
...
ipcMain.on('register-protocol-stream-handler', function (evt, scheme) {
  console.log('register-protocol-stream-handler', scheme)
  const sender = evt.sender
  if (evt.sender.isDestroyed()) {
    return
  }
  ...
  let counter = 0
  protocol.registerStreamProtocol(scheme, (request, cb) => {
    const requestId = scheme + '-' + counter++
    if (sender.isDestroyed()) {
      cb('')
    } else {
      ...
      const chunks = []
      let streamish = null
      const streamChunkCb = (evt, stringifiedBuffer) => {
        const data = JSON.parse(stringifiedBuffer).data
        const buffer = Buffer.from(data)
        chunks.push(buffer)
        // console.log(`stream DATA ${requestId}`, {buffer})
      }
      ipcMain.on(`chrome-protocol-stream-handled-${requestId}-stream`, streamChunkCb)
      ipcMain.once(`chrome-protocol-stream-handled-${requestId}-stream-finish`, (evt, data) => {
        ipcMain.removeListener(`chrome-protocol-stream-handled-${requestId}-stream`, streamChunkCb)
        const buffer = Buffer.concat(chunks)
        // console.log(`stream FINISHED ${requestId}`, buffer)
        streamish = toStream(buffer)
      })
      ipcMain.once(`chrome-protocol-stream-handled-${requestId}`, (evt, data) => {
        console.log(`chrome-protocol-stream-handled-${requestId}`, data)
        cb({
      ...
      sender.send('chrome-protocol-stream-handler-' + scheme, request, requestId)
      // TODO(bridiver) - timeout??
    }
  })
})
