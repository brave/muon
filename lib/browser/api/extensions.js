...
app.on('web-contents-created', function (event, webContents) {
  ...
  webContents.on('did-navigate', function () {
    if (webContents.getURL().startsWith('chrome-extension:')) {
      blacklistedTabs[webContents.getId()] = true
      return
    }

    for (i = 0; i < urlBlacklist.length; i++) {
      if (webContents.getURL().match(urlBlacklist[i])) {
        blacklistedTabs[webContents.getId()] = true
        return
      }
    }

    delete blacklistedTabs[webContents.getId()]
  })
  webContents.on('destroyed', function () {
