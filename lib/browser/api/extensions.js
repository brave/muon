...
var initializeTab = function (tabId, webContents) {
  ...
  if (pending) {
    ...
    try {
      pending.sender.send('chrome-tabs-create-response-' + pending.responseId, extensions.tabValue(webContents))
    } catch (e) {}
    delete pending_tabs[tabId]
  }
  sendToBackgroundPages('all', getSessionForTab(tabId), 'chrome-tabs-created', extensions.tabValue(webContents))
}
