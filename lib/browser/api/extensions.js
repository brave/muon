...
process.on('should-create-web-contents', (e, openerTab, containerType, frameName, targetUrl) => {
...
const createTabValue = function (webContents) {
  ...
  tabs[tabId].tabValue = extensions.tabValue(webContents)
...
var getSessionForTab = function (tabId) {
  ...
  return webContents && !webContents.isDestroyed() && webContents.session
}
...
const createTab = function (createProperties, sender, responseId) {
  webContents.createTab({
    ...
    const tabId = webContents.getId()
    tabs[tabId] = webContents

    if (!sender.isDestroyed()) {
      sender.send('chrome-tabs-create-response-' + responseId, getTabValue(tabId))
    }
  })
}
...
const chromeTabsUpdated = function (tabId) {
  ...
  var webContents = getWebContentsForTab(tabId)
  let tabValue = tabs[tabId].tabValue = extensions.tabValue(webContents)
  let changeInfo = {}
