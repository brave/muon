...
const getOwner = function (createProperties) {
  const openerTabId = createProperties.openerTabId
  if (openerTabId) {
    return binding.fromTabID(openerTabId)
  }
...
WebContents.prototype._init = function () {
  ...
  this.on('ELECTRON_GUEST_VIEW_MANAGER_REGISTER_GUEST', function (event, tabId) {
    guestViewManager.registerGuest(event.sender, tabId)
  })
...
// Public APIs.
module.exports = {
  createTab (createProperties, cb = null) {
    const owner = getOwner(createProperties)

    binding._createTab({
      owner,
      partition: createProperties.incognito ? "default" : "persist:default",
      src: createProperties.url
    }, (webContents) => {
      // TODO(bridiver) set active, etc..
      cb && cb(webContents)
    })
  },
