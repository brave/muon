...
WebContents.prototype._init = function () {
  ...
  if (!this.isGuest()) {
    this.on('tab-inserted-at', (event, webContents, index, active) => {
      const initialEventArgs = ['tab-inserted-at', index, active, this.id]
      guestViewManager.registerGuest(webContents, this, initialEventArgs)
    })
  }

  // hack for browserAction
  if (this.isGuest() && !this.isTab()) {
    this.on('will-attach', (event, embedder) => {
      guestViewManager.registerGuest(this, embedder)
    })
  }
