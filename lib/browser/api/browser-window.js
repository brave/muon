...
const hasPinnedTabsForWindow = (windowId) => {
  const {getTabsForWindow} = require('./extensions')
  const tabs = getTabsForWindow(windowId)
  return tabs.filter(
    (tab) => tab.pinned && tab.windowId === windowId
  ).length > 0
}

process.on('chrome-tabs-updated', (e, status, tab) => {
  if (windowIdPendingTransfer &&
      tab.windowId !== windowIdPendingTransfer &&
      (tabIdPendingTransfer && tab.windowId !== windowIdPendingTransfer ||
        !hasPinnedTabsForWindow(windowIdPendingTransfer))) {
    const wins = BrowserWindow.getAllWindows().map((win) => win.id)
    const win = BrowserWindow.fromId(windowIdPendingTransfer)
    const {getTabsForWindow} = require('./extensions')
    const tabs = getTabsForWindow(windowIdPendingTransfer)
    const shouldCloseWindow = (!tabIdPendingTransfer || tabs.length === 0) &&
      !hasPinnedTabsForWindow(windowIdPendingTransfer)

    tabIdPendingTransfer = null
    windowIdPendingTransfer = null
...
BrowserWindow.prototype.hasPendingTabMove = function () {
  if (!windowIdPendingTransfer) {
    return false
  }
  return windowIdPendingTransfer === this.id
}

var tabIdPendingTransfer
var windowIdPendingTransfer
BrowserWindow.prototype.setTabIdPendingTransfer = function (windowId, tabId) {
  windowIdPendingTransfer = windowId
  tabIdPendingTransfer = tabId
}
...
BrowserWindow.prototype._init = function () {
  ...
  this.on('close', (event) => {
    ...
    if (winCount > 1 && (tabIdPendingTransfer || hasPinnedTabsForWindow(this.id))) {
      this.pendingClose = true
      windowIdPendingTransfer = this.id
