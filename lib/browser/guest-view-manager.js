...
let guests = {}
const registerGuest = function (guest, embedder, initialEventArgs) {
  ...
  if (initialEventArgs) {
    embedder.send.apply(embedder, ['ELECTRON_GUEST_VIEW_INTERNAL_DISPATCH_EVENT', tabId, ...initialEventArgs])
  }
  ...
  const fn = function (event) {
    guest.on(event, function (_, ...args) {
      ...
      let forceSend = false
      if (event === 'tab-replaced-at') {
        const newContents = args[2]
        registerGuest(newContents, embedder)
      }
