...
binding.getWebContents = function (tabId, cb) {
  ...
  ipcRenderer.on('ELECTRON_BROWSER_GET_WEB_CONTENTS_RESPONSE_' + responseId, (evt, res) => {
    const webContents = metaToValue(res)
    webContents.dispatchEvent = function (domEvent) {
      console.log('dispatch ' + domEvent.type)
      $Function.apply(EventEmitter.prototype.emit, webContents, [domEvent.type, domEvent])
    }
    webContents.addEventListener = EventEmitter.prototype.on.bind(webContents)
    GuestViewInternal.registerEvents(webContents, tabId)
