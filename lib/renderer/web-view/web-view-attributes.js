...
// contentScripts to be run for the webview as a comma-delimited list of file names
class ContentScriptsAttribute extends WebViewAttribute {
  constructor(webViewImpl) {
    super(webViewConstants.ATTRIBUTE_CONTENT_SCRIPTS, webViewImpl);
  }
  ...
  getValue() {
    ...
    return this.webViewImpl.webviewNode.getAttribute(this.name).split(',').map(file => {
      var fileUrl = resolveURL(file);
      var protocol = fileUrl.substr(0, 5);
      if (protocol !== 'file:') {
...
WebViewImpl.prototype.setupWebViewAttributes = function() {
  ...
  this.attributes[webViewConstants.ATTRIBUTE_CONTENT_SCRIPTS] = new ContentScriptsAttribute(this);
