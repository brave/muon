...
var WEB_VIEW_EVENTS = {
  'tab-inserted-at': ['index', 'active', 'windowId'],
  'tab-moved': ['fromIndex', 'toIndex'],
  'tab-replaced-at': ['tabId', 'index', 'webContents'],
  'load-start': ['url', 'isMainFrame', 'isErrorPage'],
  'will-detach': [],
  'did-attach': ['tabId'],
  'tab-detached-at': ['windowWebContentsId', 'index'],
...
var dispatchEvent = function (tabId, eventName, ...args) {
  ...
  domEvent.eventTabId = tabId
  GuestViewInternal.emit(tabId, domEvent)
  GuestViewInternal.emit('window-tab-event', domEvent)
}
